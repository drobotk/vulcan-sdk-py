...
class HTTP:
    ...
    async def api_request( self, verb: str, url: str, **kwargs ) -> Any:
        ...
        if not res.success:
            msg = res.feedback.Message if res.feedback is not None else f"{verb} {url} {res.success = }"
    ...
    async def cufs_send_credentials( self, email: str, password: str ) -> str:
        ...
        return await self.request("POST", url, data = {"LoginName": email, "Password": password } )

    async def uonetplus_send_cert( self, symbol: str, wa: str, wctx: str, wresult: str ) -> str:
        url = self.build_url(
            module = 'uonetplus',
            path = paths.UONETPLUS.LOGIN,
            symbol = symbol
        )
        return await self.request("POST", url, data = {"wa": wa, "wctx": wctx, "wresult": wresult } )
    ...
    async def uczen_get_registers( self, symbol: str, instance: str, headers: dict[str, str] ) -> list[StudentRegister]:
        url = self.build_url(
            module = "uonetplus-uczen",
